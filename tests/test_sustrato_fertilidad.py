...
def test_ley_todo_sustrato_tiene_fertilidad_base_en_0_1():
    ...
    for numbre, propiedades in catalogo.items():
        if propiedades.get("forma_en_undo") == "sustrato":
            assert "fertilidad_base" in propiedades, f"{numbre} sin ferilidad_base"
...
def test_ley_los_tres_sustratos_nuevos_existen_con_esquema_completo():
    ...
    catalogo = datos["materiales"]
    campos_obligatorios = {
        "categoria", "forma_en_undo", "densidad_kg_m3", "dureza",
        "tasa_infiltracion", "capacidad_retencion", "combustibilidad",
        "apto_construccion", "fertilidad_base",
    }
    for numbre in SUSTRATOS_NUEVOS:
        assert numbre in catalogo, f"falta el material nuevo {numbre}"
        assert campos_obligatorios.issubset(catalogo[numbre].keys()), (
            f"{numbre} no tiene el esque ma completo de un sustato"
        )
        assert catalogo[numbre]["forma_en_mundo"] == "sustrato"


def test_ley_sustrato_por_bioma_no_cambia_de_forma_todavia():
    """Este plan NO toca sustato_por_bioma -- sigue siendo un mapeo

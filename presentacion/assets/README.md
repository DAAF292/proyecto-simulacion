...
### Segunda ronda de feedback real (2026-08-27): tamaño por dato real, no por regla mía

Diego corrigió directamente: **"El gnomo es más pequeño que el lobo en
código, la representación de las figuras debe responder a las medidas
físicas que tienen en el motor, no a una regla que tú definas."** Tenía
razón, y yo no lo había comprobado: `ESCALA_ESPECIE` (retirada) era una
constante inventada a ojo con gnomo como el más grande — exactamente al
revés de `rangos_raciales` en `config/constantes.yaml` (gnomo peso
`[8,15]`, lobo `[60,90]`). `DimensionesFisicas.peso` es literalmente el
sustituto declarado de la vieja `Categoria.tamano` (docstring de
`componentes/dimensiones_fisicas.py`) — el dato de tamaño real ya
existía, `construir_instantanea` ya lo exponía como
`entidad.dimensiones.peso` (por INDIVIDUO, no por especie: rango racial
+ sorteo propio, sin tocar el backend), y simplemente no lo estaba
usando. Sustituido por `escalaPorPeso()`: raíz cúbica del peso
normalizada contra `PESO_MAX_REFERENCIA = 90` (el máximo real de
cualquier rango racial hoy, lobo) — raíz cúbica porque si el peso fuese
proporcional a un volumen, el tamaño lineal escala con su raíz cúbica,
una relación física real, no una curva elegida a ojo. `necromasa` es la
única excepción real (`ESCALA_NECROMASA`): son restos inertes sin
`DimensionesFisicas` propio, así que no hay peso que leer — se
conservó el valor ya calibrado a ojo para esa especie.

**Reconciliado (2026-08-28) con el trabajo en paralelo de otra sesión**
sobre este mismo archivo (ver más abajo, "poses de criatura"): esa
sesión reintrodujo su propia `ESCALA_ESPECIE` (por desconocer esta
corrección) además de un `ESCALA_POSE` genuinamente nuevo y
complementario — un factor de aspecto por pose (de pie/tumbado/
durmiendo no comparten proporciones), medido con PIL sobre los sprites
reales. Al fusionar: se retiró `ESCALA_ESPECIE` (superada por
`escalaPorPeso`) y se conservó `ESCALA_POSE` intacto — ambos factores
se multiplican en `construirElementoCriatura()`, no son alternativas.

Sobre el río: esa misma sesión, en pruebas posteriores contra más
formas reales del motor, encontró que el autotile de piezas
(`dibujarRioPiezas`, documentado más arriba) seguía produciendo
serpientes de gancho repetido y lo retiró por completo, volviendo el
río a spline vectorial siempre. Es la decisión que queda en el código
final — ver "El RIO se traza PRIMERO" más abajo. El código y los PNG de
piezas de río quedan en el árbol sin usar, igual que el resto de
material retirado en este proyecto.

**Verificación:** Playwright contra pares gnomo/lobo reales
(`estado.json`), confirmando el nuevo tamaño relativo.
